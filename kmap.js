...
调试->
1.⌃+d(启动-调试),⌃+h(热-加载),⌃+s(停止),⌃+f(继续)
  ⌃+w(单步),⌃+e(跳入),⌃+r(跳出),⌃+c(至-光标),⌃+c(中断)
1.⌃+j(跑-配置),⌃+k(编辑-配置),⌃+l(eval),⌃+v(改-值)
1.⌃+b(断点s),⌃+x(行-断点),⌃+z(条件-断点),⌃+⇧+x(删-断点s)
1.⌘+b(构建),⌥+d(调-ctx),⌥+r(跑-ctx),⌃+u(模拟器)
辅助->
1.⌘+⇧+↩(补全-句),⌃+/(补全),⌃+p(函数-参数),⌥+↩(快速-修复),⌘+⌥+t(围绕)
1.⌃+i(实现-方法),⌃+o(重写-方法)
1.⌃+1(查-使用),⌃+2(重命名),⌃+7(切-收藏)
1.⌘+ml(至-定/用),⌘+mr(快-定),⌥+ml(至-类定),⌥+mr(快-类定),⌘+⌥+ml(快-doc),⌃+n(至-实现)
1.⌥+1(至-定),⌥+⇧+2(快-定),⌥+2(至-类定),⌥+3(快-doc)
1.⌃+\\(反编)
---
Finder->
